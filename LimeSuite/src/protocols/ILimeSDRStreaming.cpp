    ...
    mActive(false)
{
    ...
    pktLost = 0;
    sampleCnt = 0;
    //std::cout << "config.bufferLength forced to 1 in ILimeSDRStreaming.cpp" << std::endl; //SID
    startTime = std::chrono::high_resolution_clock::now();
    //this->config.bufferLength = 1; //SID
...
int ILimeSDRStreaming::Streamer::SetupStream(size_t& streamID, const StreamConfig& config)
{
    ...
    std::cout << "config.performanceLatency used inside ILimeSDRStreaming.cpp" << std::endl;
