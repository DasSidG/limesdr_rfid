    ...
    _moduleName(handle.module)
{
    ...
    SoapySDR::logf(SOAPY_SDR_INFO, "*************End of Constructor*****************"); //SID

}

SoapyLMS7::~SoapyLMS7(void)
{
    SoapySDR::logf(SOAPY_SDR_INFO, "Destructor");
...
{
    SoapySDR::logf(SOAPY_SDR_INFO, "setDCOffsetMode");
    std::unique_lock<std::recursive_mutex> lock(_accessMutex);
    auto rfic = getRFIC(channel);
...
{
    SoapySDR::logf(SOAPY_SDR_INFO, "getDCOffsetMode");
    std::unique_lock<std::recursive_mutex> lock(_accessMutex);
    auto rfic = getRFIC(channel);
...
void SoapyLMS7::setGain(const int direction, const size_t channel, const std::string &name, const double value)
{
    ...
    SoapySDR::logf(SOAPY_SDR_INFO, "Actual %s%s[%d] gain %g dB", dirName, name.c_str(), int(channel), this->getGain(direction, channel, name));
}
...
void SoapyLMS7::setFrequency(const int direction, const size_t channel, const std::string &name, const double frequency, const SoapySDR::Kwargs &args)
{
    ...
    if (name == "BB")
    {
        ...
        */
        ...
        SoapySDR::logf(SOAPY_SDR_INFO, "NOTE: SID SoapyLMS7::setFrequency BB frequency forced to 0"); //SID
        double freq = 0; //SID
            
        //Modified to replace 'frequency' with 'freq', where freq = 0. Don't want the BB frequency to be nonzero.
        int pos = 0, neg = 1;
        switch (direction)
        {
        case SOAPY_SDR_RX:
            if (rfic->Get_SPI_Reg_bits(LMS7_MASK, true) != 0) std::swap(pos, neg);
            rfic->Modify_SPI_Reg_bits(LMS7param(CMIX_BYP_RXTSP), (freq == 0)?1:0);
            rfic->Modify_SPI_Reg_bits(LMS7param(CMIX_SC_RXTSP), (freq < 0)?neg:pos);
            break;
        case SOAPY_SDR_TX:
            rfic->Modify_SPI_Reg_bits(LMS7param(CMIX_BYP_TXTSP), (freq == 0)?1:0);
            rfic->Modify_SPI_Reg_bits(LMS7param(CMIX_SC_TXTSP), (freq < 0)?neg:pos);
            break;
        }
        if (rfic->SetNCOFrequency(lmsDir, 0, abs(freq)) != 0)
        {
            //rate was out of bounds, clip to the maximum frequency
            const double dspRate = rfic->GetReferenceClk_TSP(lmsDir);
            rfic->SetNCOFrequency(lmsDir, 0, dspRate/2);
...
void SoapyLMS7::setSampleRate(const int direction, const size_t channel, const double rate)
{
    ...
    if (std::abs(factor-intFactor) > 0.01) SoapySDR::logf(SOAPY_SDR_INFO,
...
{
    SoapySDR::logf(SOAPY_SDR_INFO, "writeSetting");

    //Begin section added by SID
    if (key == "ENABLE_TDD")
    {
        for (size_t channel = 0; channel < _rfics.size()*2; channel++)
        {
            this->writeSetting(SOAPY_SDR_TX, channel, "ENABLE_TDD", value);
        }
        SoapySDR::logf(SOAPY_SDR_INFO, "In first writeSetting for ENABLE_TDD");
    }
    //End section add by SID
    ...
    else if (key == "CALIBRATE_TX")
    {
        for (size_t channel = 0; channel < _rfics.size()*2; channel++)
        {
            ...
            SoapySDR::logf(SOAPY_SDR_INFO, "TX calibrated"); //SID
        }
    }
    ...
    else if (key == "CALIBRATE_RX")
    {
        for (size_t channel = 0; channel < _rfics.size()*2; channel++)
        {
            ...
            SoapySDR::logf(SOAPY_SDR_INFO, "RX calibrated"); //SID
        }
    }
    ...
    else if (key == "CALIBRATE_TX_EXTLOOPBACK")
    {
        for (size_t channel = 0; channel < _rfics.size()*2; channel++)
        {
            ...
            SoapySDR::logf(SOAPY_SDR_INFO, "TX_EXTLOOPBACK calibrated"); //SID
        }
    }
    ...
    else if (key == "CALIBRATE_RX_EXTLOOPBACK")
    {
        for (size_t channel = 0; channel < _rfics.size()*2; channel++)
        {
            ...
            SoapySDR::logf(SOAPY_SDR_INFO, "RX_EXTLOOPBACK calibrated"); //SID
        }
    }
...
void SoapyLMS7::writeSetting(const int direction, const size_t channel, const std::string &key, const std::string &value)
{
     ...
     if (key == "ENABLE_TDD")
    {
        ...
        SoapySDR::logf(SOAPY_SDR_INFO, "In second writeSetting for ENABLE_TDD"); 
    }
    //End section aded by SID
    ...
    else if (key == "ENABLE_GFIR_LPF")
    {
        ...
        {
            SoapySDR::logf(SOAPY_SDR_INFO, "Entered main if statement of ENABLE_GFIR_LPF");
            //The coeffs specified are for this channel

            Coeff_Count = (uint8_t) vect.at(1);
            GFIR_Index = (uint8_t) vect.at(2);
            SoapySDR::logf(SOAPY_SDR_INFO, "Coeff count is %d", Coeff_Count);
            SoapySDR::logf(SOAPY_SDR_INFO, "GFIR Index is %d", GFIR_Index);
            ...
            {
                case 0:
                    SoapySDR::logf(SOAPY_SDR_INFO, "Case zero attempted in GFIR");
                    rfic->Modify_SPI_Reg_bits(LMS7param(GFIR1_L_RXTSP), 7); // Set L to 0 as N = 5 (L = roundUp(N/5)-1 for GFIR2) SID
                    rfic->Modify_SPI_Reg_bits(LMS7param(GFIR1_N_RXTSP), 40); // N=5 SID
                    rfic->Modify_SPI_Reg_bits(isTx?LMS7param(GFIR1_BYP_TXTSP):LMS7param(GFIR1_BYP_RXTSP), 0);
                    //rfic->Modify_SPI_Reg_bits(LMS7param(CDSN_RXATSP), 1); //Enable clock inversion //SID
                    //rfic->Modify_SPI_Reg_bits(LMS7param(CDSN_RXBTSP), 1); //Enable clock inversion //SID
                    break;
                case 1:
                    SoapySDR::logf(SOAPY_SDR_INFO, "Case one attempted in GFIR");
                    ...
                    break;
                case 2:
                    //rfic->Modify_SPI_Reg_bits(LMS7param(MAC),1); //select channel 0 (A)
                    SoapySDR::logf(SOAPY_SDR_INFO, "Case two attempted in GFIR");
                    rfic->Modify_SPI_Reg_bits(isTx?LMS7param(GFIR3_BYP_TXTSP):LMS7param(GFIR3_BYP_RXTSP), 0); 
                    rfic->Modify_SPI_Reg_bits(LMS7param(GFIR3_N_RXTSP), 75); // N=120 SID
                    rfic->Modify_SPI_Reg_bits(LMS7param(GFIR3_L_RXTSP), 5); // Set L to 7 as N = 120 (L = roundUp(N/15)-1 for GFIR3) SID
                    
                    //rfic->Modify_SPI_Reg_bits(isTx?LMS7param(GFIR1_BYP_TXTSP):LMS7param(GFIR1_BYP_RXTSP), 1); //SID
                    //rfic->Modify_SPI_Reg_bits(isTx?LMS7param(GFIR2_BYP_TXTSP):LMS7param(GFIR2_BYP_RXTSP), 1); //SID
                    //rfic->Modify_SPI_Reg_bits(LMS7param(GFIR3_L_RXTSP), 6); // Set L to 6 as N = 90 (L = N/15 for GFIR3) SID
                    //rfic->Modify_SPI_Reg_bits(LMS7param(GFIR3_N_RXTSP), 90); // N=90 SID
