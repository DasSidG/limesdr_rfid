...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/17.12/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/17.12");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("http://downloads.myriadrf.org/project/limesuite/17.12/" + name);
